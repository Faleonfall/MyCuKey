...
struct SuggestionBarSnapshotTests {
    ...
    private func state(_ cells: [SuggestionBarCell], token: String) -> SuggestionBarState {
        SuggestionBarState(
            mode: .currentToken, cells: cells, context: SuggestionContext.parse(token)!)
    }
    ...
    @Test func threeSuggestionsLight() {
        assertSnapshot(
            of: bar(state(threeCells, token: "Teh")), size: size,
    ...
    @Test func singleOriginalOnly() {
        let cells = [
            SuggestionBarCell(text: "hello", source: .userInput, role: .original, confidence: 1.0)
        ]
