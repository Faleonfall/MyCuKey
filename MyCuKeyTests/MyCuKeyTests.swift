...
struct MyCuKeyTests {
    ...
    @Test func testKeyboardCapitalizationAtStart() async throws {
        ...
        #expect(
            handler.isShiftEnabled == true, "Keyboard should automatically shift on empty fields.")
    }
    ...
    @Test func testKeyboardCapitalizationAfterSentence() async throws {
        ...
        #expect(
            ...
            "Keyboard should automatically shift after a period and space.")
    }
    ...
    @Test func testKeyboardNoCapitalizationMidSentence() async throws {
        ...
        #expect(
            handler.isShiftEnabled == false, "Keyboard should not shift in the middle of a word.")
    }
    ...
    @Test func testDoubleSpaceIsReplacedWithDot() async throws {
        ...
        let firstPress = handler.evaluateTextInsertion(
            text: " ", context: "Hello", now: time1, lastPress: nil)
        #expect(firstPress.textToInsert == " ", "First space should insert normally.")
        #expect(firstPress.needsDeleteBackward == false)

        let secondPress = handler.evaluateTextInsertion(
            text: " ", context: "Hello ", now: time2, lastPress: firstPress.newLastSpacePress)
        #expect(
            secondPress.textToInsert == ". ", "Fast double space should insert a period and space.")
        #expect(secondPress.needsDeleteBackward == true)
        #expect(
            secondPress.newLastSpacePress == nil, "Press timer must reset after a period is placed."
        )
    }
    ...
    @Test func testSlowDoubleSpaceIsIgnored() async throws {
        ...
        let secondPress = handler.evaluateTextInsertion(
            text: " ", context: "Hello ", now: time2, lastPress: firstPress.newLastSpacePress)
        #expect(
            secondPress.textToInsert == " ",
            "Slow double space should just insert another normal space.")
        #expect(secondPress.needsDeleteBackward == false)
        #expect(
            secondPress.newLastSpacePress == time2, "Timer should reset to this new slow press.")
    }
    ...
    @Test func testDoubleSpaceBoundaryAtExactlyPoint25SecondsIsIgnored() async throws {
        ...
        #expect(
            secondPress.textToInsert == " ",
            "At exactly 0.25s, replacement should not trigger because threshold is strict (< 0.25)."
        )
        #expect(secondPress.needsDeleteBackward == false)
    }
    ...
    @Test func testKeyboardCapitalizationAfterExclamationAndQuestionMark() async throws {
        ...
        #expect(
            handler.isShiftEnabled == true,
            "Keyboard should shift after exclamation mark and space.")

        handler.isShiftEnabled = false
        handler.evaluateAutoCapitalization(contextBefore: "Really? ")
        #expect(
            handler.isShiftEnabled == true, "Keyboard should shift after question mark and space.")
    }
    ...
    @Test func testSpaceTapTimerResetByOtherKey() async throws {
        ...
        let firstPress = handler.evaluateTextInsertion(
            text: " ", context: "Hello", now: time1, lastPress: nil)
        let intermediatePress = handler.evaluateTextInsertion(
            text: "A", context: "Hello ", now: time2, lastPress: firstPress.newLastSpacePress)

        #expect(intermediatePress.textToInsert == "A")
        #expect(
            intermediatePress.newLastSpacePress == nil,
            "Typing any other letter must wipe the double-space timer.")
    }
    ...
    @Test func testKeyboardCapitalizationWithAsterisksAndSpacelessTerminators() async throws {
        ...
        handler.evaluateAutoCapitalization(contextBefore: "Hello. *")
        #expect(
            handler.isShiftEnabled == true, "Must shift after dot, space, asterisk combination.")

        handler.evaluateAutoCapitalization(contextBefore: "Hello *")
        #expect(
            handler.isShiftEnabled == false,
            "Must NOT shift after a raw asterisk unconnected to punctuation.")
    }
    ...
    @Test func testKeyboardCapitalizationAfterBulletPrefixOnCurrentLine() async throws {
        ...
        #expect(
            handler.isShiftEnabled == true,
            "Must shift when a bullet asterisk is the whole current line.")

        handler.evaluateAutoCapitalization(contextBefore: "* ")
        #expect(
            handler.isShiftEnabled == true,
            "Must keep shift enabled after bullet asterisk followed by a space.")

        handler.evaluateAutoCapitalization(contextBefore: "Line one\n*")
        #expect(
            handler.isShiftEnabled == true, "Must shift when a bullet asterisk starts a new line.")

        handler.evaluateAutoCapitalization(contextBefore: "Line one\n* ")
        #expect(
            handler.isShiftEnabled == true,
            "Must keep shift enabled after a bullet asterisk and space on a new line.")
    }
    ...
    @Test func testKeyboardCapitalizationDisablesAfterTypingNormalLetters() async throws {
        ...
        handler.evaluateAutoCapitalization(contextBefore: "Hello world")
        #expect(
            handler.isShiftEnabled == false, "Typing normal letters must un-shift the keyboard.")
    }
    ...
    @Test func testKeyboardCapitalizationWithMultiplePunctuationMarks() async throws {
        ...
        #expect(
            handler.isShiftEnabled == true, "Multiple dots ending in a trigger should capitalize.")

        handler.evaluateAutoCapitalization(contextBefore: "Really?!")
        #expect(
            handler.isShiftEnabled == true,
            "Compound punctuation should recognize the exclamation mark.")
    }
